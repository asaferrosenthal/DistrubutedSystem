...
public class LFU_Server
  ...
  static int serverPageFaultCounter = 0;
  ...
  static int[][] setCurrentRAMFreq() {
    ...
    for (int i = 0; i < frameSize; i++) {
      ...
      //currentRAMFreq[i][1] = Integer.MIN_VALUE;
    }
  ...
  static int faultCheck(int pageNum, int receiveFromPort) {
    ...
    if (pageFault == true)
    {
      ...
      // There is a page fault and the page elements reach the max frame size
        ...
        currentRAM.remove(Integer.valueOf(LFUPage));
        currentRAM.add(HDDResult);
        // Updating current RAM pageFrequencies
        for (int[] pageFreq : currentRAMFreq) {
          if (pageFreq[0] == LFUPage) {
  ...
  // Applies LFU algorithm to get the least frequently used page
  static int getLFU()
  {
    int leastFreq = Integer.MAX_VALUE;
    int LFUPage = -1; // Placeholder value
    // Finding LFU Page
    for (int[] pageFreq : currentRAMFreq) {
      if (pageFreq[1] < leastFreq) {
        LFUPage = pageFreq[0];
        leastFreq = pageFreq[1];
      }
    }
    return LFUPage;
  }
